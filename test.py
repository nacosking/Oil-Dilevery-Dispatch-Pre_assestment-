...
check("has lat and lng",             depot["lat"] is not None and depot["lng"] is not None)
...
# Try again — should fail (already arrived)
...
print("\n▸ build_coord_map()")
check("coords map not empty",        len(coords) > 0)
check("depot has coordinates",       1 in coords)
check("lat/lng are floats",          all(isinstance(c[0], float) for c in coords.values()))


# ─────────────────────────────────────────────────────────────────────────────
print("\n══ Task 3a: Dijkstra (graph.py) ══\n")
# ─────────────────────────────────────────────────────────────────────────────
...
check("returns None for unreachable", path2 is None)
...
print("▸ astar()")
a_path, a_dist = astar(graph, coords, 1, 7)
check("finds path depot -> id=7",    a_path is not None)
check("path starts at depot (1)",    a_path[0] == 1)
check("path ends at target (7)",     a_path[-1] == 7)
check("distance is 40.5 km",        a_dist == 40.5)

a_path2, a_dist2 = astar(graph, coords, 1, 11)
check("returns None for unreachable", a_path2 is None)
check("returns None dist for unreachable", a_dist2 is None)

print("\n▸ A* vs Dijkstra — same results?")
for dest in [2, 3, 4, 5, 6, 7, 8, 9, 10]:
    d_path, d_dist = dijkstra(graph, 1, dest)
    a_path, a_dist = astar(graph, coords, 1, dest)
    check(f"depot -> {dest} same path & distance", d_path == a_path and d_dist == a_dist)

print("\n▸ find_unreachable_customers()")
all_locs    = get_all_locations()
cust_ids    = [l["id"] for l in all_locs if l["type"] == "customer"]
unreachable = find_unreachable_customers(graph, coords, depot["id"], cust_ids)
