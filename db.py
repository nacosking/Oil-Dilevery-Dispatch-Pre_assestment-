...
def get_delivery_by_id(delivery_id: int, *, db=None):
    ...
    """
    if db:
        row = db.execute(sql, (delivery_id,)).fetchone()
        return dict(row) if row else None
    else:
        with get_db() as conn:
...
def get_truck_plates():
    """Return all distinct truck plates from past deliveries."""
    sql = """
        SELECT DISTINCT truck_plate
        FROM deliveries
        ORDER BY truck_plate
    """
    with get_db() as conn:
        rows = conn.execute(sql).fetchall()
        return [row["truck_plate"] for row in rows]